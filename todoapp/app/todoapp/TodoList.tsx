...
function NewTodoItem({ onAdded }: { onAdded: (item: TodoItem) => void }) {
    ...
    return (
        <div id='todo_new_item'>
            <input value={text} onChange={printText} onKeyDown={handleKeyDown} maxLength={120} />
 ...
 */
function TaskItemList({ name, notes, toRowText, error, setError, clearErrors, onDelete, onMarkFinished }: GenericTodoItemListProps) {
            ...
            {notes.map((note) => (
                <div className="todo_list_row" key={note.id} >
                    <div className="todo_task_buttons">
                        {onMarkFinished && <Button className='button_mark_done' buttonOnClick={() => {markAsDone(note.id)}}/>}
...
export function TodoList() {
	...
	const fetchFinished = () => {
        ...
        axios.get<TodoItem[]>("/api/notes?status=FINISHED&orderBy=finishingTime&order=DESC")
            .then(res =>
                 setFinishedNotes(res.data)
            ).catch(err => {
    ...
    useEffect(() => {
        ...
        axios.get<TodoItem[]>("/api/notes?status=UNFINISHED&orderBy=createTime&order=ASC")
            .then(res => 
                setNotes(res.data)
            ).catch(err => {
    ...
    const handleAdded = (item: TodoItem) =>
    ...
    return (
        ...
        <TaskItemList 
			...
			toRowText={(note) => (
				<>
					{note.finishingTime && <>Finished at: {formatDateTime(note.finishingTime)}<br/></>}
					{note.text}
				</>
			)}
